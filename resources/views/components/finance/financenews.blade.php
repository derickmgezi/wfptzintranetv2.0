...
<div class="row">
    <div class="col-md-12" style="background-color:">
        ...
        <a class="btn btn-success btn-lg float-right hidden-sm-down {{ $editor? '':'invisible'}}" href="{{URL::to('/create_update')}}" role="button">
            <i class="fa fa-plus-square" aria-hidden="true"></i> Add Update
        </a>
        <a class="btn btn-success float-right hidden-md-up {{ $editor? '':'invisible'}}" href="{{URL::to('/create_update')}}" role="button">
            <i class="fa fa-plus-square" aria-hidden="true"></i> Add Update
        </a>
    ...
    <div class="col-md-6{{ ($update_count%2 == 1)?'':' push-md-6' }}">
        ...
        <blockquote class="blockquote{{ ($update_count%2 == 1)?' blockquote-reverse':'' }}">
            <p class="text-left"> 
                ...
                <span class="badge badge-pill badge-primary" data-delay="300" data-trigger="hover" data-container="body" data-toggle="popover" data-placement="right" data-html="true" title="Liked By" data-content="@if($total_unique_like_count == 0)No likes @else @foreach($unique_likes as $like){{ App\User::find($like->liked_by)->firstname.' '.App\User::find($like->liked_by)->secondname }}<br> @endforeach @endif">
...
<!-- Add News Modal -->
{{Form::open(array('url' => '/store_update/'.$department.'/'.$dutystation,'enctype' => "multipart/form-data",'role' => 'form'))}}
<div class="modal fade add-update-modal" id="add-update-modal" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header bg-success text-white">
